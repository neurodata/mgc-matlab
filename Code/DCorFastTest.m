%% Fast unbiased dcorr test that runs in O(n log n), based on 
...
%% @export
%%
%% This script is the recommended version for fast DCor / Hsic test, which has almost same power as the permutation test but very fast.
%%
function  [corr,pval]=DCorFastTest(X,Y,opts)
% check input
if nargin<3
...
opts = struct('metric',optionMetric,'center','unbiased','fast',1,'max',optionMax);
[corr,~]=DCor(X,Y,opts);
...
switch optionMax
    ...
    case 1
        pval=1-chi2cdf(corr*n+1,1)^p;
    case p
        pval=1-chi2cdf(corr*n*p+p,p);
    otherwise
        pval=0;
