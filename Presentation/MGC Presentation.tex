...
\definecolor{UniBlue}{RGB}{250,250,250}%{0,26,56}
\definecolor{UniOrange}{RGB}{0,26,56}%{253,185,19}
\definecolor{UniTitle}{RGB}{0,0,0}%{192,192,192}
\definecolor{UniText}{RGB}{0,0,0}%{200,200,200}
\definecolor{OutCirc}{RGB}{125,154,197}%{185,224,247}
   ...
   \makebox[\textwidth][c]{%
      \begin{tikzpicture}[
         ...
         every node/.append style={align=left}]
				...
				\node (source2)at (3,3){$d(\cdot,\cdot)$};
				\pause
...
\textbf{Input:} $\mathcal{X}_{n}=[x_{1},\ldots,x_{n}]$ as the data matrix with each column representing one sample observation, and similarly $\mathcal{Y}_{n}$. A distance or kernel function $d(\cdot,\cdot)$, by default the Euclidean distance.\\
\pause
\medskip
